...
{
  mid_MsgQueue_lcd = osMessageQueueNew(MSGQUEUE_OBJECTS_lcd, sizeof(MSGQUEUE_OBJ_lcd_t), NULL);
...
{
	while (1){
		status = osMessageQueueGet(mid_MsgQueue_lcd, &msg, NULL, 0U);
    if (status == osOK) {
			cargarMSG(msg);
		}
		osThreadYield();  
...
void cargarMSG(MSGQUEUE_OBJ_lcd_t msg)
{
	int i;
	limpiarPantalla();
	sprintf(cadena, " %s" , msg.linea_1);
	for(i = 0; i < strlen(cadena); i ++){
		symbolToLocalBuffer_L1(cadena[i],i);
	}
	sprintf(cadena, " %s" , msg.linea_2);
	for(i = 0; i < strlen(cadena); i ++){
