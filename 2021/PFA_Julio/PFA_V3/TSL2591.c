...
#define STATUS_AVALID_MASK 0x01
...
void Th_I2C (void *argument) {
	...
	uint32_t flags;
	//Escribir en A0 03 
	uint8_t data[] = {TSL_CMD_BIT_REG|TSL_TRANSACTION|ENA_REG, ALS_ON|POWER_ON, MEDIUM_GAIN};
	write_TSL2591(data, 3);
	while (1) {
		osThreadFlagsWait(INIT_LIGHT_MEASURE|INIT_LIGHT_CYCLE, osFlagsWaitAny | osFlagsNoClear, osWaitForever);
		flags = osThreadFlagsGet();
 		if((flags&INIT_LIGHT_MEASURE)>>28){
			...
			i=((flags&N_CYCLE_MASK) >> N_CYCLE_SHIFT) + 1;
			t=((flags&N_TIME_MASK) >> N_TIME_SHIFT);
			do{
				if(!capture_lum(true))i--;
