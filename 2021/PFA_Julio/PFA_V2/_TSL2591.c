...
void Th_I2C (void *argument) {
	...
	const uint32_t _max_lum = MAX_VALUE_VIS - MIN_VALUE_VIS;	//Fixed value
	//Escribir en A0 03 
  ...
  while (1) {
		...
		if((lum_buf[0] & STATUS_AVALID_MASK) == true){
			...
			msg.light_uni = ((measured_data/_max_lum) %1000);//measured_data - msg.light_cen - msg.light_dec;
