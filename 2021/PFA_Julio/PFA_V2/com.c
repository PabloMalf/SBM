...
void USART3_callback(uint32_t event){
	...
	if(event & ARM_USART_EVENT_TX_COMPLETE) {
		...
		//v1//osThreadFlagsSet(tid_pc_com_Thread, ARM_USART_EVENT_TX_COMPLETE);
		//V2
	...
	while(1){
		...
		if(cmd[0] == INIT_DATAGRAM){	//&&((proc_flags & ARM_USART_EVENT_RECEIVE_COMPLETE)!= 0)
			...
			if(cmd[size] == END_DATAGRAM){
				...
				}while((queue_status == osOK && msg_tx.last_msg == false && osMessageQueueGetCount(mid_MsgQueue_com_tx)!=0));	
