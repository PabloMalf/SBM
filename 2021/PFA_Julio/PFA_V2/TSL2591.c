...
int Init_Th_I2C (void) {
  ...
  return(Init_I2C_message_queue());
}
 
void Th_I2C (void *argument) {
	uint8_t lum_buf[4];
	uint32_t measured_data;
	//Escribir en A0 03 
  ...
  while (1) {
		osThreadFlagsWait(INIT_LIGHT_MEASURE, osFlagsWaitAny | osFlagsNoClear, osWaitForever);	//Se queda esperando hasta que recibe un flag del OS
		...
		if((lum_buf[0] & STATUS_AVALID_MASK) == true){
			...
			msg.light_cen = ((measured_data/(MAX_VALUE_VIS - MIN_VALUE_VIS)) / 100);
			msg.light_dec = ((measured_data/(MAX_VALUE_VIS - MIN_VALUE_VIS)) %100)/10;//(measured_data - msg.light_cen) % 10;
			msg.light_uni = ((measured_data/(MAX_VALUE_VIS - MIN_VALUE_VIS)) %1000)>>12;//measured_data - msg.light_cen - msg.light_dec;
			
			osMessageQueuePut(mid_MsgQueue_I2C, &msg, 0U, 0U);	
			osThreadFlagsClear(INIT_LIGHT_MEASURE);
		}
  }
}
