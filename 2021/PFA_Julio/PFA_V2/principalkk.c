			...
			case MEDIDA:
				...
				if(lock == UNLOCKED){//En mitad de ciclo medidas
					...
					if((status_msg == osOK) && (msg_joy.tecla == Abajo) && (msg_joy.duracion == Corta)){
						...
						sprintf(msg_lcd.data_L2, "Luz: %d%d", msg_i2c.light_cen, msg_i2c.light_dec);
					}
					//COMS
					status_msg = osMessageQueueGet(mid_MsgQueue_com_rx, &msg_com_rx, NULL, 0U);
					if(status_msg == osOK){
...
static void ret_all_measure(MSGQUEUE_OBJ_COM msg){
	//if(buffer->elementos==buffer->capacidad)	//Nuestro buffer esta lleno
	//Primero supongamos que el buffer nunca se lleno. En ese caso tenemos que devolver desde la posicion 0 a la posicion elementos
	/*v1
	if(buffer->elementos < buffer->capacidad){
		for(int i=0; i<buffer->elementos; i++){
			msg.Number=buffer->elementos-i;
			measureToMsg(&msg, i);
			osMessageQueuePut(mid_MsgQueue_com_tx, &msg, NULL, 0U);
		}
	}
	...
	msg.last_msg=false;
	if(buffer->elementos == buffer->capacidad){
		i=buffer->ultima_entrada;
	}
	do{
		i=(i+1)%buffer->capacidad;
		if(i!=buffer->ultima_entrada)msg.last_msg=true;
		measureToMsg(&msg, i);
		osMessageQueuePut(mid_MsgQueue_com_tx, &msg, NULL, 0U);
	}while(msg.last_msg);
}
